...
# MODIFIED: Removed OpenAI and Anthropic, added Gemini
import google.generativeai as genai
from transformers import AutoTokenizer, AutoModelForCausalLM

# MODIFIED: Cleaned up imports, assuming load_api_key now handles the Gemini key
...
#### GEMINI MODEL (REPLACED FROM OPENAI) ####
...
def generate_molecules_for_protein(protein, input_csv, output_dir, api_key, model_engine, gnina_path, config_path, temp_dir, 
    ...
    Returns:
        None
    """
    # MODIFIED: Removed OpenAI client
    ...
    # This function uses a single pass, but the loop is kept for structure consistency.
    for iteration in range(1, max_iterations + 1):
        # The original code had a 'context_text' variable here that was not defined in this function.
        # It's assumed to be empty for non-context functions.
        context_text = ""
        messages = [
                {
                    "role": "system", 
        ...
        try:
            # MODIFIED: Swapped OpenAI API call for Gemini API call
            ...
            raw_generated_smiles = response.text.strip()
...
def generate_molecules_for_protein_multifactors(protein, input_csv, output_dir, api_key, model_engine, gnina_path, config_path, temp_dir, 
    ...
    for iteration in range(1, max_iterations + 1):
        context_text = "" # Placeholder for consistency
        messages = [
                {
                    "role": "system", 
                    "content": (
...
# MODIFIED: Removed the entire CLAUDE MODEL section to standardize on Gemini
# #### CLAUDE MODEL ####
# def generate_molecules_for_protein_multifactors_with_context_claude(...):
#     ... (function code was here) ...


#### MOLECULE GPT2 ####
# This section is unchanged as it uses a local model.
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        for i, seed in enumerate(scaffolds):
            ...
            outputs = model.generate(
                ...
                do_sample=True,
                top_p=0.9,
                temperature=0.5, #deterministic 0.3, 0.5, 0.7, 0.8 (default), 0.9 stochastic
...
if __name__ == "__main__":
    ...
    # MODIFIED: This now loads the Gemini key by default
    api_key = load_api_key()
    # MODIFIED: Default model is now gemini-pro
    model_engine = "gemini-2.5-pro"
    ...
    if mode == "1":
        ...
        generate_molecules_for_protein_multifactors(
            ...
            config_path=config_path,
            temp_dir=temp_dir,
            parameter_ranges={'CNNaffinity': affinity_range, 'MolWt': (100, 700)},
            target_size=10,
            max_iterations=10,
            max_samples=5
        )
